...
def head_to_adj(sent_len, head, label, len_, mask, directed=False, self_loop=True):
    """
...
def aux_to_adj(sent_len, head, label, len_, masks, directed=False, self_loop=True):
    '''
    masks [aspect_num, seq_len]
    '''
    adj_matrix = np.zeros((sent_len, sent_len), dtype=np.float32)
    label_matrix = np.zeros((sent_len, sent_len), dtype=np.int64)
    ...
    for i in range(len(masks)):
        asp_idx = [idx for idx in range(len(masks[i])) if masks[i][idx] == 1]
        asp_list.append(asp_idx)
    for idx, head in enumerate(head):
        for asp_idx in asp_list:
            if idx in asp_idx:
                for k in asp_idx:
                    adj_matrix[idx][k] = 1
                    label_matrix[idx][k] = 2
